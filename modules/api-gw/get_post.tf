...
resource "aws_api_gateway_method_response" "rest_api_get_method_response_400" {
  rest_api_id     = aws_api_gateway_rest_api.rest_api.id
  resource_id     = aws_api_gateway_resource.rest_api_resource.id
  http_method     = aws_api_gateway_method.rest_api_get_method.http_method
  status_code     = 400
...
resource "aws_api_gateway_integration" "rest_api_get_method_integration" {
  ...
  type                    = "AWS"
  uri                     = var.get_post_lambda_function_arn
...
resource "aws_api_gateway_integration_response" "rest_api_get_integration_response_200" {
  depends_on = [
...
resource "aws_api_gateway_integration_response" "rest_api_get_integration_response_400" {
  depends_on = [
    aws_api_gateway_integration.rest_api_get_method_integration
  ]
  rest_api_id       = aws_api_gateway_rest_api.rest_api.id
  resource_id       = aws_api_gateway_resource.rest_api_resource.id
  http_method       = aws_api_gateway_method.rest_api_get_method.http_method
  status_code       = aws_api_gateway_method_response.rest_api_get_method_response_400.status_code
...
resource "aws_lambda_permission" "api_gateway_lambda" {
  ...
  function_name = var.get_post_lambda_function_name
